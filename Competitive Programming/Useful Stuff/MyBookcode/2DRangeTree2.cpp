...
struct node1 { // inside
	...
	int query(int ql, int qr, int sl, int sr) {
		...
		int md = ((sl + sr < 0) ? (sl + sr - 1) / 2 : (sl + sr) / 2);
		return max((C[0] ? C[0]->query(ql, qr, sl, md) : (overlap(ql, qr, sl, md) ? INIT : -INF)),
			(C[1] ? C[1]->query(ql, qr, md + 1, sr) : (overlap(ql, qr, md + 1, sr) ? INIT : -INF)));
	}
};
...
struct node2 { // outside
	...
	void update(int x, int y, int v, int sl, int sr) {
		...
		int ans = max((C[0] ? C[0]->val->query(y, y, -INF, INF) : INIT),
			(C[1] ? C[1]->val->query(y, y, -INF, INF) : INIT));
	...
	int query(int qxl, int qxr, int qyl, int qyr, int sl, int sr) {
		...
		int md = ((sl + sr < 0) ? (sl + sr - 1) / 2 : (sl + sr) / 2);
		return max((C[0] ? C[0]->query(qxl, qxr, qyl, qyr, sl, md) : (overlap(qxl, qxr, sl, md) ? INIT : -INF)),
			(C[1] ? C[1]->query(qxl, qxr, qyl, qyr, md + 1, sr) : (overlap(qxl, qxr, md + 1, sr) ? INIT : -INF)));
	}
};
