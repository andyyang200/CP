...
pii a[MAXN];
multiset<int> byX[MAXN + 10];
multiset<int> byY[MAXN + 10];
class SegmentTree
{
public:
	vector<int>* st;
...
int main(void)
{
	...
	//            FOR(k, 0, n){
	//                if (k == i || k == j) continue;
	//                if (a[k].first >= min(a[i].first, a[j].first) && a[k].first <= max(a[i].first, a[j].first)
	//                 && a[k].second >= min(a[i].second, a[j].second) && a[k].second <= max(a[i].second, a[j].second)){
	//                     bad = true;
	//                     break;
	//                }
	//            }
