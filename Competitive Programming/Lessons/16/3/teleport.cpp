...
/*
You have a N by M grid. Each square has a value. Find the maximum value path through the grid, following
these rules:

Your path must start in the first row and end in the last row.
You can move southwest, south, or southeast each move.
Additionally, some squares of the grid have a teleportation device. At such a square,
you have the additional option of moving to any square with a teleportation device(including the location you were at).
You can use a teleportation device at most K times.

Print out the maximum value possible, mod 10^9 + 7.
N, M <= 1500
K <= 10^9
*/
