...
struct node2 { // outside
	...
	void update(int x, int y, int v, int sl, int sr) {
		...
		int ans = max((C[0] ? C[0]->val->query(y, y, 0, n) : 0),
			(C[1] ? C[1]->val->query(y, y, 0, n) : 0));
		val->update(y, ans, 0, n);
	}

	int query(int qxl, int qxr, int qyl, int qyr, int sl, int sr) {
		if (sl >= qxl && sr <= qxr) {
			return val->query(qyl, qyr, 0, n);
		}
...
int main(void)
{
    ...
    node2 root;
    FOR(i, 0, n){
        int x = root.query(0, a[i] - 1, 0, b[i] - 1, 0, n) + 1;
        root.update(a[i], b[i], x, 0, n);
        ans = max(ans, x);
    }
    cout << ans << endl;
}
